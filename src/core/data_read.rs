...
use crate::constants::get_db_file;
use crate::core::util::decode;
use crate::core::core::*;
...
pub fn get_values_binary(name: String) -> Result<(String, Vec<u8>), ErrorType> {
    ...
    let header = get_storage_header(name.clone()).unwrap();
    {
        ...
        for i in values {
            let mut g = i.split("-");
            mapped_vals.push((g.next().unwrap().to_string(), g.next().unwrap().to_string()));
        }
        ...
        loop {
            ...
            if *check.first().unwrap() != OBJECT_SEPARATOR {
                break;
            }
            ...
            for i in mapped_vals.clone() {
                match i.1.as_str() {
                    "int" => {
                        ...
                        obj.insert(i.0, LDBValue::INT(int));
                    }
                    "string" => {
                        ...
                        obj.insert(i.0, LDBValue::STRING(strng));
                    }
                    "bool" => {
                        let mut buf = [0; 1];
                        match file.read(&mut buf) {
                        ...
                        obj.insert(i.0, LDBValue::BOOL(bl));
                    }
                    _ => return Err(ErrorType::InvalidFormat),
                }
            }
...
pub fn get_storage_address(storage_name: String) -> Result<u64, ErrorType> {
    let mut metafile = OpenOptions::new()
        .read(true)
        .append(true)
        .open(crate::constants::get_tbl_file())
        .unwrap();
    Ok(loop {
